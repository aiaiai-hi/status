...
# части воронки (3 сегмента внутри каждого уровня)
funnel_parts = {
...
CHART_H = 200   # высота графиков
MARGIN  = dict(t=10, b=32, l=42, r=8)

# ── утилиты ────────────────────────────────────────────────────────────────
def base_layout(height=CHART_H):
    return dict(
        height=height,
        margin=MARGIN,
...
def section_title(text):
    st.markdown(
        f"""<div style="background:{BLUE};color:#fff;font-size:15px;
                        font-weight:500;border-radius:8px;padding:8px 14px;
                        margin-bottom:14px;display:inline-block;">{text}</div>""",
        unsafe_allow_html=True,
    )
...
with tab1:
    ...
    # ── ROW 1: 2 метрики | График пораженности ─────────────────────────
    m1, g1 = st.columns([1, 3])
    with m1:
    ...
    with g1:
        ...
        fig.update_layout(**base_layout(), title_text="Динамика по неделям средней пораженности",
                          title_font_size=12, title_x=0)
        st.plotly_chart(fig, use_container_width=True)

    st.divider()

    # ── ROW 2: 2 метрики | График доли устранённых ─────────────────────
    m2, g2 = st.columns([1, 3])
    with m2:
    ...
    with g2:
        ...
        fig2.update_layout(**base_layout(), title_text="Доля устранённых отклонений (за 8 недель)",
                           title_font_size=12, title_x=0)
        st.plotly_chart(fig2, use_container_width=True)

    st.divider()

    # ── ROW 3: 1 метрика | График счётчика повторов ────────────────────
    m3, g3 = st.columns([1, 3])
    with m3:
    ...
    with g3:
        ...
        fig3.update_layout(**base_layout(),
                           title_text="Динамика среднего счётчика повторов",
                           title_font_size=12, title_x=0)
        st.plotly_chart(fig3, use_container_width=True)

    st.divider()

    # ── ROW 4: 1 метрика | График скорости ─────────────────────────────
    m4, g4 = st.columns([1, 3])
    with m4:
    ...
    with g4:
        ...
        fig4.update_layout(**base_layout(), title_text="Скорость устранения по неделям",
                           title_font_size=12, title_x=0)
...
with tab2:
    ...
    with mc1:
        st.metric("Доля по Банку", "50,1%", "+0,2%")
    with mc2:
        st.metric("Целевое значение", "—")
    with mc3:
        st.metric("Средний счётчик повторов", "5,6 нед.", "-0,2")
    with mc4:
        st.metric("Кол-во задач / хроник", "—")
    with mc5:
        st.metric("Доля устранённых", "—")
    with mc6:
        st.metric("Скорость устранения", "—")

    st.divider()

    # ── Основная секция: [графики 2/3] | [воронка 1/3] ─────────────────
    left_col, right_col = st.columns([2, 1])

    with left_col:
        # График 1 — динамика показателей
        fig5 = go.Figure()
        fig5.add_trace(go.Bar(
            name="ВСП", x=bmo_weeks, y=bmo_vsp,
            marker_color=BAR_B, opacity=0.8,
            yaxis="y"
        ))
        fig5.add_trace(go.Scatter(
            name="Задачи", x=bmo_weeks, y=bmo_tasks,
            mode="lines+markers", line=dict(color=LINE_C, width=2),
        ...
        # График 2 — доля и скорость
        segments = ["Тип А", "Тип Б", "Тип В", "Тип Г"]
        fig6 = go.Figure()
        ...
        fig6.add_trace(go.Scatter(
            ...
            mode="lines+markers", line=dict(color=LINE_A, width=2),
            marker=dict(size=5)
        ))
        fig6.update_layout(
            **base_layout(height=220),
            title_text="Доля и скорость устранения",
            title_font_size=12, title_x=0,
            showlegend=True,
            legend=dict(orientation="h", y=1.15, x=0, font=dict(size=10)),
            barmode="overlay",
    ...
    with right_col:
        st.markdown(
            "<div style='font-size:12px;color:#5F5E5A;margin-bottom:8px;'>Воронка метрик БА по БМО</div>",
            unsafe_allow_html=True
        )
        # Горизонтальная воронка с разбивкой на части (stacked bar)
        fig_funnel = go.Figure()

        parts_keys = list(FUNNEL_COLORS.keys())
        for part in parts_keys:
            fig_funnel.add_trace(go.Bar(
                ...
                orientation="h",
                marker_color=FUNNEL_COLORS[part],
                opacity=0.85,
        ...
        # Итоговые подписи справа
        total_annots = []
        for i, stage in enumerate(funnel_stages):
            total_annots.append(dict(
                x=funnel_vals[i] + 30,
                y=stage,
